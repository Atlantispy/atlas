...
OUTPUT_ROOT = HERE.parents[1]/'outputs'/'terrain-model-r5-shoreline'
...
FORCING_FIELDS = {'steps','dt_years','dx_m','dy_m','external_outlets','connectivity',
                  'cover_scale_m','settling_m_year','basin_settling_m_year',*VECTOR_FIELDS}
...
_MODULES={}
_NAMESPACE='_r5_shoreline_bound_'+uuid.uuid4().hex+'_'
...
def validate(recipe):
    ...
    if recipe['schema']!='diadem.terrain.shoreline.recipe.r5' or recipe['purpose']!='SYNTHETIC_ENGINEERING_ONLY':
        raise ValueError('only synthetic R5 shoreline recipes are authorised')
...
def run(recipe_path,output,*,resume=False,interrupt_after_step=None,interrupt_at=None):
    ...
    if output==allowed or not output.is_relative_to(allowed) or recipe_path.is_relative_to(allowed):
        raise ValueError('output outside bounded R5 shoreline root or contains its recipe')
    ...
    identity={'recipe_sha256':_sha(raw),'canonical_recipe_sha256':_sha(io.canonical(recipe)),
              ...
              'mode':'general_native_shoreline_reference_r5'}
    ...
    with _writer_lock(output.with_name('.'+output.name+'.writer.lock')):
        ...
        try:
            ...
            for i,name in enumerate(names):
                ...
                checkpoint={'schema':'diadem.terrain.shoreline.checkpoint.r5','generation_id':identity['generation_id'],
            ...
            result={'schema':'diadem.terrain.shoreline.result.r5','generation_id':identity['generation_id'],
            ...
            receipt={'schema':'diadem.terrain.shoreline.receipt.r5','identity':identity,'products':products,
                     'physical_acceptance':False,'production_authorised':False}
