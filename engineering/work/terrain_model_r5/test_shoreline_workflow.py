...
def recipe():
    return {'schema':'diadem.terrain.shoreline.recipe.r5','purpose':'SYNTHETIC_ENGINEERING_ONLY',
...
class PersistenceTests(unittest.TestCase):
    def setUp(self):
        temporary=tempfile.TemporaryDirectory(prefix='r5-shoreline-workflow-')
        self.addCleanup(temporary.cleanup);self.root=Path(temporary.name)
        self.allowed=self.root/'outputs'/'terrain-model-r5-shoreline'
    ...
    def test_beyond_endpoint_hydraulic_blocker_cannot_become_success(self):
        ...
        with self.assertRaisesRegex(ValueError,'BLOCKED_FORCED_DAUGHTER_COUPLING') as caught:
            w.run(self.input,self.output())
