...
def write_checkpoint(output, recipe, report):
    ...
    allowed = Path(__file__).resolve().parents[2] / "outputs" / "terrain-model-r1-repair-r1"
    if not output.resolve().is_relative_to(allowed.resolve()) or output.resolve() == allowed.resolve():
        raise ValueError("Output must be a new child of this task's outputs/terrain-model-r1-repair-r1.")
