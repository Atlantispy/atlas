"""Source-captured, single-preflight orchestration over unchanged R26 science."""
import hashlib
...
if sys._getframe().f_code != compile(_SOURCE, __file__, 'exec', dont_inherit=True):
    raise ValueError('executed R28 bootstrap differs from current source')
...
class _Finder(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        ...
        if not name.isidentifier():
            raise ImportError('flat R28 module identity required')
        source = HERE/(name+'.py')
        if not source.is_file():
            raise ImportError('undeclared R28 module: '+fullname)
