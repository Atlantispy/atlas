...
class Context:
    ...
    def group(self, layer):
        return {f["properties"][ID_KEYS[layer]]: f for f in self.layers[layer]}
...
def _fault_graph(c):
    ...
    for nid, node in nodes.items():
        ...
        actual = {fid for fid, fault in faults.items() if c.geom(node).distance(c.geom(fault)) <= EPS_KM}
        incident[nid] = actual
...
def _crown(c):
    ...
    for tid, feature in titan.items():
        centre = c.geom(feature).centroid
    ...
    for mid, feature in massifs.items():
        ...
        distance = anchors[haus].distance(geom.centroid)
...
def _sections(c):
    ...
    for sid, feature in traces.items():
        ...
        entries = [f for f in c.sections if f["properties"].get("section_id") == sid]
        ...
        if sid == "X3":
            stations = [trace.project(c.geom(by_haus[name]).centroid) for name in required_routes[sid]]
