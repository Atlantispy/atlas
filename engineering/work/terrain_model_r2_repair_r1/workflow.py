...
_IMPORT_ROOT=HERE
# Capture the workflow and loader before loading exact private dependencies.
# Do not resolve them through mutable process-global module names.
_WORKFLOW_BYTES=Path(__file__).read_bytes()
_BINDING_PATH=HERE/"source_binding.py"
_BINDING_BYTES=_BINDING_PATH.read_bytes()
_binding=types.ModuleType("_terrain_r2_repair_source_binding")
_binding.__file__=str(_BINDING_PATH)
exec(compile(_BINDING_BYTES,str(_BINDING_PATH),"exec"),_binding.__dict__)
_RUNTIME_MODULES,_LOADED_RUNTIME_PINS=_binding.load(HERE,{
    "workflow.py":_WORKFLOW_BYTES,"source_binding.py":_BINDING_BYTES})
_core=_RUNTIME_MODULES["core.py"]
Grid,State,advance,constraints_check,field,gradients,number=(getattr(_core,name) for name in
    ("Grid","State","advance","constraints_check","field","gradients","number"))
constructive=_RUNTIME_MODULES["constructive.py"]
materials=_RUNTIME_MODULES["materials.py"]
water=_RUNTIME_MODULES["water.py"]
basin_topology=_RUNTIME_MODULES["basin_topology.py"]
MAX_FILE=16*1024*1024
MAX_PRODUCT_BYTES=64*1024*1024


def verify_loaded_sources():
    return _binding.verify(_IMPORT_ROOT,_LOADED_RUNTIME_PINS)
...
def execute(recipe):
    ...
    for op in recipe["operations"]:
        ...
        if kind=="coupled":
            ...
            stress_history={key:layers["organic"][key] for key in
                ("effective_stress_pa","compaction_branch","compaction_parameters")
                if "organic" in layers and key in layers["organic"]}
            layers["organic"]={**stress_history,"height_m":aux,"organic_kg":[t.organic_after_kg for t in transfers],
            ...
            if "effective_stress_pa" in layer:
                if list(stresses["effective_stress_before_pa"])!=layer["effective_stress_pa"]:
                    raise ValueError("compaction must consume the previous effective stress; unloading/reset is unsupported")
                if layer["compaction_branch"]!="monotonic_virgin" or layer["compaction_parameters"]!=asdict(p):
                    raise ValueError("existing compaction branch/material parameters cannot silently change")
            ...
            layer["effective_stress_pa"]=list(stresses["effective_stress_after_pa"])
            layer["compaction_branch"]="monotonic_virgin"
            layer["compaction_parameters"]=asdict(p)
...
def run(recipe_path,output,*,resume=False,interrupt_at=None,expected_recipe_sha256=None):
    ...
    loaded_pins=verify_loaded_sources()
    ...
    allowed=(HERE.parents[1]/"outputs"/"terrain-model-r2-repair-r1",)
    ...
    identity={"recipe_sha256":hashlib.sha256(data).hexdigest(),"implementation":pins,
              "import_loaded_runtime":loaded_pins,"python":sys.version,
              "platform":platform.platform(),"mode":"reference_r2_repair_r1"}
