"""Versioned evolving terrain over Physical's selected, unchanged R18 seed.
...
from work.generator_upgrade_r18 import consumer, accounts, working
from work.generator_runtime_r12.store import Store
from . import provenance as p

SCHEMA = 'diadem.evolving-terrain-state.r22'
VIEW_SCHEMA = 'diadem.current-terrain-view.r22'
...
def _binding():
    return {'r22': p.identity(moving_ground=True), 'r18': consumer.p.identity(),
...
def verify(envelope):
    ...
    binding = envelope['binding']; p.verify(binding['r22']); consumer.p.verify(binding['r18'])
...
def advance(envelope, forcing, *, store=None):
    body = verify(envelope); before = _native(body['current_state'])
    ...
    if result is None:
        ...
        namespace = dict(consumer.terrain_step.__globals__, verify=checked_view, _lineage=lineage)
        method = FunctionType(consumer.terrain_step.__code__, namespace, consumer.terrain_step.__name__,
            consumer.terrain_step.__defaults__, consumer.terrain_step.__closure__)
...
def view(envelope):
    ...
    context['snapshot_id'] = 'R22_TERRAIN_CHILD_'+envelope['body_sha256'] if body['history'] else context['snapshot_id']
