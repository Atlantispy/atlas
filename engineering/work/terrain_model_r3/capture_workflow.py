...
No production or domain acceptance. R2 and its installed task remain unchanged.
"""
import argparse
import hashlib
import json
...
import capture
from r2_io import io, verify_dependencies
...
def validate(recipe):
    ...
    if recipe["schema"]!="diadem.terrain.capture.recipe.r3" or recipe["purpose"]!="SYNTHETIC_ENGINEERING_ONLY":
...
def verify_output(directory,identity):
    ...
    if result["schema"]!="diadem.terrain.capture.result.r3" or result["scenario_id"]!=recipe["scenario_id"] or result["checkpoints"]!=checkpoint_names:
...
def run(recipe_path,output,*,resume=False,interrupt_after_step=None,interrupt_at=None):
    ...
    allowed=HERE.parents[1]/"outputs"/"terrain-model-r3"
    ...
    identity={"recipe_sha256":hashlib.sha256(raw).hexdigest(),"canonical_recipe_sha256":hashlib.sha256(io.canonical(recipe)).hexdigest(),"implementation":pins,"python":sys.version,
              "platform":platform.platform(),"mode":"prescribed_port_capture_reference_r3"}
    ...
    result={"schema":"diadem.terrain.capture.result.r3","scenario_id":recipe["scenario_id"],"state":state.as_dict(),
