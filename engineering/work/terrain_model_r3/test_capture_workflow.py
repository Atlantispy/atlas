...
fixture. Numerical modules and their frozen R2 dependencies remain read-only.
These tests establish no Diadem, shoreline or physical acceptance.
"""
...
def small_recipe():
    r = deepcopy(capture_fixtures.suite()[0])
...
class ValidationTests(unittest.TestCase):
    def test_six_fixture_recipes_are_valid_and_independent(self):
        recipes = capture_fixtures.suite()
        self.assertEqual(len(recipes), 6)
        self.assertEqual(len({r["scenario_id"] for r in recipes}), 6)
...
class PersistenceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.here = self.root / "work" / "terrain_model_r3"
        ...
        self.inputs.mkdir()
        self.allowed = self.root / "outputs" / "terrain-model-r3"
