"""Exact-source R6 regression evidence. Never a production/scientific release.
...
if __name__=='__main__':
    ...
    _raw=_path.read_bytes();_helper=_types.ModuleType('_r6_exact_verification_bootstrap')
...
ROOT=HERE.parents[1]/'outputs'/'scientific-corrections-2026-09-09'
...
def main(argv=None,*,runtime=None):
    if runtime is None or not runtime.installed:
        raise ValueError('R6 verification requires the fresh exact-source bootstrap')
    ...
    if files!=inventory['files']:raise ValueError('R6 test file inventory drift')
    ...
    if loader.errors or ids!=inventory['test_ids'] or digest(ids)!=inventory['test_ids_sha256']:
        raise ValueError('R6 complete test discovery identity drift')
    runtime.check()
    ...
    evidence={'schema':'diadem.terrain.r6.scientific-correction-verification.v1',
