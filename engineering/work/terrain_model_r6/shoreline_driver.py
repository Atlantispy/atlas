...
def _trial_impl(state, dt, parameters, counts, deadline, *, finalise=False):
    ...
    base, hill = hillslope_trial(state, dx_m=p['dx_m'], dy_m=p['dy_m'],
                ...
                critical_gradient=p.get('critical_gradient'))
    ...
    channel_args['erosion_reference_runoff_m_year']=p.get('erosion_reference_runoff_m_year')
    ...
    for row in pools:
        ...
        if row['qw'] == row['qs'] == row['qo'] == 0:
            ...
            counts['trial_cell_evaluations'] += state.size
            allowance=MAX_CELL_TRIALS-counts['trial_cell_evaluations']
            if allowance<0:raise CouplingError('nested closed-pool cell envelope exhausted')
            try:
                closed, closed_report = event_settling.settle_components(subset,
                            ...
                            max_cell_visits=min(262144,allowance))
            except Exception as exc:
                counts['closed_event_solver_calls'] += getattr(exc,'solver_calls',0)
                counts['trial_cell_evaluations'] += getattr(exc,'cell_visits',0)
                raise
            counts['closed_event_solver_calls'] += closed_report['solver_calls']
            counts['trial_cell_evaluations'] += closed_report['cell_visits']
        ...
        if (pool['liquid_m3'] or pool['suspended_solid_m3']) and any(
            ...
            if detected['status'] == 'FIRST_DRYING_ESTIMATED' and exact_endpoint is None:
                ...
                raise CouplingError('BLOCKED_UNCERTIFIED_DRYING_EVENT',
                    status='BLOCKED_UNCERTIFIED_DRYING_EVENT',pool_id=pool['id'],
            ...
            if detected['status'] != 'NO_DRYING_CERTIFIED' and exact_endpoint is None:
                raise CouplingError(detected['status'],status=detected['status'],
                    ...
                    unchanged_driver_checkpoint=state.as_dict())
            if exact_endpoint is None and detected.get('independent_no_drying_certificate',{}).get('status')!='PROVED_POSITIVE_DEPTH':
                raise CouplingError('BLOCKED_UNCERTIFIED_CONTINUATION',pool_id=pool['id'],
                                    unchanged_driver_checkpoint=state.as_dict())
...
def advance(state, *, steps, dt_years, dx_m, dy_m, external_outlets, runoff_m_year,
    ...
    parameters['critical_gradient'] = (None if critical_gradient is None else
        tuple(capture.number(v,'critical gradient',0,True) for v in capture.vector(critical_gradient,state.size,'critical gradient',0)))
    parameters['erosion_reference_runoff_m_year'] = (None if erosion_reference_runoff_m_year is None else
        capture.number(erosion_reference_runoff_m_year,'erosion reference runoff',0,True))
