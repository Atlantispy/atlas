...
def _sum(values, name):
    try:
        ...
        raise ContinuousPoolError(name+': sum exceeds binary64', failure_kind='ARITHMETIC_UNREPRESENTABLE') from exc
    if not math.isfinite(result):
        raise ContinuousPoolError(name+': nonfinite sum', failure_kind='ARITHMETIC_UNREPRESENTABLE')
    return result
...
def _product(a, b, name):
    ...
    if not math.isfinite(result) or (a and b and result == 0):
        raise ContinuousPoolError(name+': product overflow/underflow', failure_kind='ARITHMETIC_UNREPRESENTABLE')
    return result
...
def _ratio(a, b, name):
    ...
    if not math.isfinite(result) or (a and result == 0):
        raise ContinuousPoolError(name+': ratio overflow/underflow', failure_kind='ARITHMETIC_UNREPRESENTABLE')
    return result
...
"""
    ...
    def check_bound():
        if time.monotonic()-started > MAX_WALL_SECONDS:
            raise ContinuousPoolError('continuous reservoir wall-time envelope exceeded', failure_kind='RESOURCE_EXHAUSTED')

    def rhs(state):
        nonlocal evaluations
        if _work.get('evaluation_limit') is not None and evaluations >= _work['evaluation_limit']:
            exc = ContinuousPoolError('continuous reservoir caller evaluation budget exhausted', failure_kind='RESOURCE_EXHAUSTED')
            exc.evaluation_budget_exhausted = True
            raise exc
        evaluations += 1
        _work['evaluations'] = evaluations
        if evaluations > MAX_EVALUATIONS:
            raise ContinuousPoolError('continuous reservoir evaluation envelope exceeded', failure_kind='RESOURCE_EXHAUSTED')
    ...
    def rk4(state, step):
        half = step/2
        if half == 0:
            raise ContinuousPoolError('RK stage time underflows', failure_kind='ARITHMETIC_UNREPRESENTABLE')
        a = rhs(state)
    ...
    if duration == 0:
        ...
        if water < 0:
            raise ContinuousPoolError('requested interval passes pure-liquid drying event', failure_kind='PHASE_DOMAIN_FAILURE')
        ...
        while now < duration:
            ...
            if attempts > MAX_ATTEMPTS:
                raise ContinuousPoolError('continuous reservoir attempt envelope exceeded', failure_kind='RESOURCE_EXHAUSTED')
            remaining = duration-now
            step = min(step, remaining)
            if step <= 0 or now+step == now:
                raise ContinuousPoolError('positive reservoir step is below time resolution; event outside fixed-set scope', failure_kind='ARITHMETIC_UNREPRESENTABLE')
            try:
                ...
                for a, b, old in zip(coarse, refined, state):
                    ...
                    if local_budget <= 0:
                        raise ContinuousPoolError('local error budget underflows', failure_kind='ARITHMETIC_UNREPRESENTABLE')
...
def advance_pool(liquid_m3, suspended_solid_m3, area_m2,
                 liquid_input_m3_year, solid_input_m3_year,
                 mixture_export_m3_year, settling_m_year, dt_years, *, max_evaluations=None):
    ...
    try:
        if max_evaluations is not None:
            if type(max_evaluations) is not int or not 0 <= max_evaluations <= MAX_EVALUATIONS:
                raise ContinuousPoolError('caller evaluation budget outside hard envelope')
            work['evaluation_limit'] = max_evaluations
