...
class ExactEndpointTests(unittest.TestCase):
    ...
    def test_one_ulp_near_endpoint_is_not_promoted_to_exact(self):
        ...
        # The exact invariant now proves positive mathematical depth, but the
        # durable binary64 wet footprint still rounds to dry: retain that gate.
        with self.assertRaisesRegex(driver.CouplingError,'wet footprint dries'):
    ...
    def test_longer_interval_remains_blocked_without_claiming_completion(self):
        state, kwargs = forced_existing_pool()
        with self.assertRaisesRegex(driver.CouplingError,'BLOCKED_UNCERTIFIED_DRYING_EVENT') as caught:
            driver.advance(state, **kwargs)
        self.assertEqual(caught.exception.last_valid_state, state.as_dict())
    ...
    def test_non_equilibrium_forcing_is_not_certified(self):
        ...
        # This safe numerical estimate is outside the sufficient invariant.
        # R6 must not promote it to mathematical certification or continuation.
        with self.assertRaisesRegex(driver.CouplingError,'NO_DRYING_ESTIMATED') as caught:
            driver.advance(state, **kwargs)
        self.assertEqual(caught.exception.last_valid_state,state.as_dict())
        self.assertEqual(caught.exception.coupling_counts['accepted_substeps'],0)
