"""Execute the unchanged repaired-R2 hillslope kernel from exact pinned bytes."""
import hashlib
...
ROOT=Path(__file__).resolve().parent.parent/'terrain_model_r2_repair_r1'
PINS={'core.py':'b91a65135aaa49e67da5c26388d0d72142d32ee1229ec5d2e5da405b8c16fb45',
...
kernel=types.ModuleType('_r6_repaired_hillslope_'+uuid.uuid4().hex)
kernel.__file__=str(ROOT/'core.py')
sys.modules[kernel.__name__]=kernel
exec(compile(RAW['core.py'],kernel.__file__,'exec',dont_inherit=True,optimize=0),kernel.__dict__)
...
def verify():
    ...
    return [{'name':'R2-repaired-hillslope/'+name,'bytes':len(raw),'sha256':PINS[name]}
