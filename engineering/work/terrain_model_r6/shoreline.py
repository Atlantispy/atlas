...
def channel_trial(state, *, receivers, link_lengths_m, contributing_area_m2,
                  ...
                  erosion_reference_runoff_m_year=None):
    ...
    reference_runoff=(None if erosion_reference_runoff_m_year is None else
        _number(erosion_reference_runoff_m_year,'erosion reference runoff',0,True))
    ...
    for i in order:
        ...
        # sqrt(Q)/sqrt(Rref) avoids overflow/underflow of the intermediate
        # Q/Rref when the square-root intensity is still representable.
        forcing_length=(math.sqrt(upstream_area[i]) if reference_runoff is None else
                        _number(math.sqrt(water[i])/math.sqrt(reference_runoff),
                                'discharge-normalised forcing length',0,True))
        intensity = _product(forcing_length, slope, name="channel intensity")
    ...
    return result, {
        ...
        "erosion_forcing_model": ('LEGACY_CONTRIBUTING_AREA_FIXED_RUNOFF_PROXY' if reference_runoff is None
                                  else 'DISCHARGE_NORMALISED_BY_EXPLICIT_REFERENCE_RUNOFF'),
        "erosion_reference_runoff_m_year": reference_runoff,
