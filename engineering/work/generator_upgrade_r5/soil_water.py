"""R5: retained mixed Richards equations with explicit temporal failure evidence.
...
R3's integration/acceptance equations and user limits are unchanged. R5 does
not silently reduce min_dt, relax tolerances or remove thin material cells.
"""
...
def advance(column, state, forcing, boundary, controls, *, water_density_kg_m3, gravity_m_s2):
    ...
    base = {"schema": "diadem.layered-richards.r5", "state": None, "layers": None, "ledger": None,
...
def state_to_json(state):
    return json.dumps({"schema": "diadem.richards-state.r5", **asdict(state)}, sort_keys=True, allow_nan=False)
...
def state_from_json(raw, column):
    ...
    if set(data) != {"schema", "head_m", "elapsed_seconds", "column_sha256"} or data["schema"] != "diadem.richards-state.r5":
