"""Source-bound R9 entry point; sealed biome, soil and physical parents are immutable.
...
RECIPE_SCHEMA = 'diadem.species-spatial-recipe.r9'
RESULT_SCHEMA = 'diadem.species-spatial-result.r9'
CHECKPOINT_SCHEMA = 'diadem.species-spatial-checkpoint.r9'
...
def module_nodes(sources):
    ...
    for path,digest in sources.items():
        ...
        if source.parent!=provenance.HERE:
            raise ValueError('bounded R9 Python module layout must be flat')
        logical='work.generator_upgrade_r9.'+source.stem
        if logical in nodes: raise ValueError('duplicate R9 private module identity')
...
class Bundle:
    def __init__(self):
        ...
        self.graph=self.parent.graph.__class__(module_nodes(self.identity['r9_sources']))

    def verify(self):
        self.graph.verify()
        current,digest=provenance.source_identity(self.parent)
        if current!=self.identity or digest!=self.source_sha256:
            raise ValueError('R9 source inventory changed after binding')
        return digest

    @property
    def pipeline(self):
        return self.graph.load('work.generator_upgrade_r9.pipeline')

    @property
    def reference(self):
        return self.graph.load('work.generator_upgrade_r9.reference')
    ...
    def checkpoint(self, result):
        ...
        if (type(result) is not dict or result.get('schema')!=RESULT_SCHEMA
            ...
            raise ValueError('this exact source-bound R9 result required')
