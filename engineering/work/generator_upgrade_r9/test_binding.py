...
class BindingTests(unittest.TestCase):
    ...
    def test_exact_parent_seal(self):
        self.assertEqual(p.sha(p.checked(p.R8_SEAL)),p.R8_SEAL_SHA256)

    def test_exact_twenty_r8_sources(self):
        self.assertEqual(len(p.retained_record()['source_identity']['r8_sources']),20)

    def test_actual_parent_identity(self):
        self.assertEqual(self.bundle.parent.verify(),p.R8_SOURCE_SHA256)
    ...
    def test_external_ledger_ignores_preloaded_sources(self):
        name='work.generator_upgrade_r9.sources'; fake=types.ModuleType(name)
        fake.source_bindings=lambda:[]
        with patch.dict(sys.modules,{name:fake}):
            records,sources=p.reference_bindings(self.bundle.parent,self.bundle.identity['r9_sources'])
    ...
    def test_owner_provider_must_be_captured(self):
        sources=dict(self.bundle.identity['r9_sources']); sources.pop(str(p.HERE/'owner_inputs.py'))
    ...
    def test_external_ledger_live_source_change_rejected(self):
        ...
        with patch.object(p,'checked',side_effect=checked):
            with self.assertRaisesRegex(ValueError,'external bytes changed'):
                p.reference_bindings(self.bundle.parent,self.bundle.identity['r9_sources'])

    def test_actual_nested_solver_and_climate_graph(self):
        parent=self.bundle.parent.parent.parent
    ...
    def test_r8_graph_executes_exact_source(self):
        parent=self.bundle.parent
        actual=parent.pipeline
        self.assertEqual(Path(actual.__file__),p.R8_ROOT/'pipeline.py')
        record=parent.graph.executed['work.generator_upgrade_r8.pipeline']
        self.assertEqual(record['sha256'],parent.identity['r8_sources'][record['path']])

    def test_no_global_predecessor_modules(self):
        self.assertFalse(any(k.startswith(('work.generator_upgrade_r8.','work.generator_upgrade_r7.','work.generator_upgrade_r6.')) for k in sys.modules))

    def test_no_private_namespace_leak(self):
        self.assertFalse(any(k.startswith(('_diadem_r6_','_r9_exact_r8_')) for k in sys.modules))

    def test_public_graph_ignores_preloaded_module(self):
        name='work.generator_upgrade_r9.provenance'
        ...
        self.assertEqual(self.bundle.graph.executed[name]['sha256'],self.bundle.identity['r9_sources'][str(p.HERE/'provenance.py')])
    ...
    def test_changed_r9_inventory_rejected(self):
        altered=copy.deepcopy(self.bundle.identity); altered['r9_sources']['unexpected']='0'*64
    ...
    def test_checkpoint_refuses_r8_envelope(self):
        with self.assertRaises(ValueError): self.bundle.checkpoint({'schema':'diadem.biomes-vegetation-result.r8'})
