...
def _bound_sources():
    ...
    with _SOURCE_CACHE_LOCK:
        ...
        for name in SOURCE_PINS:
            ...
            exec(compile(source_bytes[name], module.__file__, "exec",dont_inherit=True,optimize=0), module.__dict__)
            modules[name] = module
...
def _distribute(total, weights):
    ...
    # Preserve an exactly representable proportional allocation. Rounding the
    # ratio before multiplication can perturb an already exact routed state
    # (including its equilibrium concentration and simultaneous drying tie).
    # If any native share needs rounding, retain the audited legacy path.
    exact_weights = [Fraction(v) for v in weights]
    exact_total = sum(exact_weights, Fraction())
    exact_shares = [Fraction(total)*v/exact_total for v in exact_weights]
    exact_result = [float(v) for v in exact_shares]
    if all(math.isfinite(v) and Fraction(v) == q for v,q in zip(exact_result,exact_shares)):
...
def _exact_capacity_allocation(wr, sr, weights, capacity, error_bound, liquid_anchor,
    ...
    if remainder:
        for i in sorted(range(len(values)), key=lambda j: (math.ulp(values[j]), j)):
            ...
            if allow_liquid_remainder and not any(sr):
                # The caller must finish an exact pure-liquid projection and
                # persist its rational residual; this is not a phase transfer.
                evidence['exact_capacity_matched'] = False
                evidence['requires_liquid_remainder'] = True
                return evidence
...
def ratio(value):
    """Canonical, JSON-safe signed rational (never a rounded decimal)."""
    return [value.numerator, value.denominator]


def liquid_remainders(water, values=None):
    """Validate the bounded durable correction to each binary64 liquid store.

    A correction is at most half one ULP of its positive represented store.
    Dry cells have exactly zero correction. Canonical integer pairs make a
    checkpoint independently auditable and exclude hidden unbounded stocks.
    """
    if values is None:
        return tuple(Fraction() for _ in water)
    if type(values) not in (list, tuple) or len(values) != len(water):
        raise ValueError('liquid remainder requires one rational pair per cell')
    result = []
    for w, pair in zip(water, values):
        if (type(pair) not in (list, tuple) or len(pair) != 2 or
                any(type(v) is not int or v.bit_length() > 8192 for v in pair) or pair[1] <= 0):
            raise ValueError('liquid remainder requires bounded canonical integer pairs')
        residual = Fraction(*pair)
        if list(pair) != ratio(residual):
            raise ValueError('liquid remainder must be canonical')
        if (w == 0 and residual) or abs(residual) > Fraction(math.ulp(w))/2 or Fraction(w)+residual < 0:
            raise ValueError('liquid remainder exceeds its half-ULP representation bound')
        result.append(residual)
    return tuple(result)


def route_phases(bed_m, area_m2, shape, outlets, liquid_m3, suspended_solid_m3, *, connectivity=4,
                 liquid_remainder_m3=None, exact_liquid=False):
    ...
    residuals = liquid_remainders(liquid, liquid_remainder_m3)
    if type(exact_liquid) is not bool or exact_liquid and liquid_remainder_m3 is None:
        raise ValueError('exact liquid routing requires explicit durable remainder storage')
    rational_liquid = any(residuals) or exact_liquid
    if rational_liquid and any(solid):
        raise UnsupportedPhaseRouting('nonzero liquid remainder with suspension requires exact phase transport')
    ...
    try:
        ...
        for leaf in leaves:
            cells = groups[leaf]
            volumes[leaf] = sum((Fraction(liquid[i]) + residuals[i] + Fraction(solid[i]) for i in cells), Fraction())
        ...
        exact_direct = sum((Fraction(liquid[i]) + residuals[i] + Fraction(solid[i]) for i in direct), Fraction())
        ...
        exact_pool_stages, before_capacity_allocation = {}, {}
        for name in active_ids:
            ...
            exact_pool_stages[name] = exact_eta
            ...
            representation = None
            before_capacity_allocation[name] = tuple(wr)
            if v > 0 and full(name):
                ...
                representation = _exact_capacity_allocation(wr, sr, weights, volumes[name], allocation_error, w,
                    allow_liquid_remainder=liquid_remainder_m3 is not None and not any(solid))
                rational_liquid = rational_liquid or representation.get('requires_liquid_remainder', False)
        ...
        out_residuals = [Fraction()] * n
        if rational_liquid:
            # Pure liquid is its own exact geometric volume. Project each cell
            # independently to the nearest float plus <= half-ULP rational;
            # their exact sum is the original routing volume, including debt.
            # Bed, sill, depth and wet-connectivity decisions are unchanged.
            for pool in pools:
                cells = pool['cell_indices']; eta = exact_pool_stages[pool['id']]
                before = before_capacity_allocation[pool['id']]
                for i in cells:
                    exact = Fraction() if eta is None else max(eta-fbed[i], 0)*farea[i]
                    out_w[i] = _float(exact)
                    out_residuals[i] = exact-Fraction(out_w[i])
                if sum((Fraction(out_w[i])+out_residuals[i] for i in cells), Fraction()) != volumes[pool['id']]:
                    raise ValueError('durable exact liquid projection changed routing inventory')
                pool['liquid_m3'] = _sum(out_w[i] for i in cells)
                pool['liquid_remainder_m3'] = ratio(sum((Fraction(out_w[i])+out_residuals[i] for i in cells), Fraction())-Fraction(pool['liquid_m3']))
                pool['liquid_allocation_residual_m3'] = _check(pool['liquid_m3']-pool['pre_output_representation_liquid_m3'], pool['liquid_m3'])
                pool['phase_representation_residual_m3'] = _check(pool['liquid_m3']-pool['mixture_m3'],pool['liquid_m3'])
                rep = pool['full_sill_representation']
                if rep is not None:
                    adjustment = sum((Fraction(out_w[i])-Fraction(w) for i,w in zip(cells,before)), Fraction())
                    adjustment_bound = Fraction(rep['arithmetic_bound_m3'])+sum((Fraction(math.ulp(out_w[i]))/2 for i in cells), Fraction())
                    if abs(adjustment) > adjustment_bound:
                        raise ValueError('exact liquid projection exceeded its arithmetic certificate')
                    rep.update(exact_capacity_matched=True, durable_liquid_remainder=True,
                               cell_offset=None, canonical_anchor='pure_liquid_plus_rational_remainder',
                               liquid_remainder_m3=pool['liquid_remainder_m3'],
                               adjustment_liquid_m3=float(adjustment),
                               adjustment_liquid_bound_m3=float(_upper_bound(adjustment_bound)))
                    rep['cell_adjustments'] = [{'phase':'liquid','cell_offset':j,'before_m3':w,'after_m3':out_w[i],
                        'adjustment_m3':float(Fraction(out_w[i])-Fraction(w))} for j,(i,w) in enumerate(zip(cells,before)) if out_w[i] != w]
            liquid_remainders(out_w, [ratio(v) for v in out_residuals])
        ...
        export_residual = Fraction()
        if rational_liquid:
            export_w = _float(exact_export)
            export_residual = exact_export-Fraction(export_w)
            liquid_remainders([export_w], [ratio(export_residual)])
            if sum((Fraction(w)+r for w,r in zip(out_w,out_residuals)), Fraction())+Fraction(export_w)+export_residual != sum((Fraction(w)+r for w,r in zip(liquid,residuals)), Fraction()):
                raise ValueError('durable exact liquid routing ledger failed')
        exact_input = sum((Fraction(w) + r + Fraction(s) for w, r, s in zip(liquid, residuals, solid)), Fraction())
        ...
        ledger = {
            ...
            "exact_liquid_remainder_accounted": bool(rational_liquid),
            "input_liquid_remainder_m3": ratio(sum((Fraction(w)+r for w,r in zip(liquid,residuals)), Fraction())-Fraction(input_w)),
            "stored_liquid_remainder_m3": ratio(sum((Fraction(w)+r for w,r in zip(out_w,out_residuals)), Fraction())-Fraction(stored_w)),
            "exported_liquid_remainder_m3": ratio(export_residual),
        ...
        return {"status": "PASS_NUMERICAL_CONNECTED_PHASE_STORAGE_ONLY", "liquid_m3": out_w,
                "liquid_remainder_m3": [ratio(v) for v in out_residuals],
                "exported_liquid_remainder_m3": ratio(export_residual),
                ...
                "assumptions": {"porosity": 0., "material_classes": 1,
                                "liquid_remainder": "OPTIONAL_EXACT_PURE_LIQUID_ONLY_MAX_HALF_ULP_PER_POSITIVE_CELL",
