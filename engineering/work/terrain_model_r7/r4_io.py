...
_R2_MODULES = {}
_R2_NAMESPACE = '_r6_exact_r2_'+uuid.uuid4().hex+'_'
...
def verify_dependencies():
    ...
    r6_raw=io.read_bytes(here/'R6_PREDECESSOR.json')
    if hashlib.sha256(r6_raw).hexdigest()!='06c5716b584748b050b9da4f819efc82c5883913fe1e4c3b345282ce339abfaa':
        raise ValueError('R7 frozen R6 binding changed')
    r6=io.parse_json(r6_raw)
    r6_dir=io.unlinked(here/r6['source_directory'])
    if sorted(p.name for p in r6_dir.iterdir() if p.is_file())!=sorted(row['name'] for row in r6['files']):
        raise ValueError('R6 predecessor inventory changed')
    r6_files=[]
    for row in r6['files']:
        if Path(row['name']).name!=row['name']:raise ValueError('invalid R6 predecessor child')
        pin=_receipt_pin(r6_dir/row['name'],row['sha256'],'R6/'+row['name'])
        if pin['bytes']!=row['bytes']:raise ValueError('R6 predecessor size changed')
        r6_files.append(pin)
    r5=io.parse_json(io.read_bytes(here/'R5_PREDECESSOR.json'))
    if (r5.get('schema')!='diadem.terrain.r6.predecessor.v1'
            or r5.get('source_directory')!='../terrain_model_r5'
            or r5.get('receipt_sha256')!='ae1c4916c19bbeafd5d1782d51e5872065929618a9505c1d3976bf8049e319c9'
            or r5.get('physical_acceptance') is not False or r5.get('production_authorised') is not False):
        raise ValueError('R6 frozen R5 predecessor binding differs')
    r5_dir=io.unlinked(here/r5['source_directory'])
    if sorted(p.name for p in r5_dir.iterdir() if p.is_file())!=sorted(row['name'] for row in r5['files']):
        raise ValueError('R5 predecessor inventory changed')
    r5_files=[]
    for row in r5['files']:
        if Path(row['name']).name!=row['name']:raise ValueError('invalid R5 predecessor child')
        pin=_receipt_pin(r5_dir/row['name'],row['sha256'],'R5/'+row['name'])
        if pin['bytes']!=row['bytes']:raise ValueError('R5 predecessor size changed')
        r5_files.append(pin)
    r5_receipt=_receipt_pin(here/r5['receipt_path'],r5['receipt_sha256'],'R5/RECEIPT.json')
    ...
    return [*r6_files,r5_receipt,*r5_files,r4_receipt,*r4_files,r3_receipt,r2_receipt,*r3_files,*_r3_io.verify_dependencies(),*hillslope_binding.verify()]
