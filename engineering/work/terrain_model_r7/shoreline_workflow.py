...
OUTPUT_ROOT = HERE.parents[1]/'outputs'/'terrain-model-r7-shoreline'
...
_MODULES={}
_NAMESPACE='_r7_shoreline_bound_'+uuid.uuid4().hex+'_'
...
def validate(recipe):
    ...
    if recipe['schema']!='diadem.terrain.shoreline.recipe.r7' or recipe['purpose']!='SYNTHETIC_ENGINEERING_ONLY':
        raise ValueError('only explicit synthetic R7 shoreline recipes are authorised')
...
def run(recipe_path,output,*,resume=False,interrupt_after_step=None,interrupt_at=None):
    ...
    if output==allowed or not output.is_relative_to(allowed) or recipe_path.is_relative_to(allowed):
        raise ValueError('output outside bounded R7 shoreline root or contains its recipe')
    ...
    identity={'recipe_sha256':_sha(raw),'canonical_recipe_sha256':_sha(io.canonical(recipe)),
              ...
              'mode':'general_native_shoreline_reference_r7'}
    ...
    with _writer_lock(output.with_name('.'+output.name+'.writer.lock')):
        ...
        try:
            ...
            for i,name in enumerate(names):
                ...
                checkpoint={'schema':'diadem.terrain.shoreline.checkpoint.r7','generation_id':identity['generation_id'],
            ...
            result={'schema':'diadem.terrain.shoreline.result.r7','generation_id':identity['generation_id'],
            ...
            receipt={'schema':'diadem.terrain.shoreline.receipt.r7','identity':identity,'products':products,
                     'physical_acceptance':False,'production_authorised':False}
