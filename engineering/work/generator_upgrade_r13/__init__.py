"""Source-captured coupled seasonal soil successor; preserves R1-R12."""
...
if _sys._getframe().f_code != compile(_BOOTSTRAP_RAW, __file__, 'exec', dont_inherit=True):
    raise ValueError('executed R13 bootstrap differs from current source')
...
class _Finder(_abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        ...
        if not name.isidentifier():
            raise ImportError('flat R13 module identity required')
        source = _ROOT / (name + '.py')
        if not source.is_file():
            raise ImportError('undeclared R13 module: ' + fullname)
