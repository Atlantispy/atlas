...
ROOT = HERE.parents[1] / 'outputs' / 'terrain-model-r4-repair-r1'
...
EXPECTED_TEST_MODULES = (
    ...
    'test_forced_drying','test_phase_storage','test_release_shoreline_reference','test_repairs',
    'test_shoreline','test_shoreline_driver','test_shoreline_materials',
    'test_shoreline_network','test_shoreline_verification',
)
EXPECTED_TEST_FILES = tuple(name+'.py' for name in EXPECTED_TEST_MODULES)
EXPECTED_TEST_IDS_SHA256 = 'b54abc2d7098fd2a2ce747f0383bdf355b5512b036b4c8e43d70956698880106'
EXPECTED_TIMED_REPORT_SHAPE_SHA256 = '9f05d1fe273c007b59e825299ea82cf1ed58aeea2d11869e155208a2ad3c262e'
EXPECTED_SCIENTIFIC_REPORT_SHAPE_SHA256 = '6b0270bb6f38357a74b09b1dba0eef6d9c001cd2c7ec0624b88d4197184aa1a4'
...
RECEIPT_KEYS = {'identity','products','test_workers','scientific_result_sha256',
...
def _validate_worker(worker,identity):
    ...
    if (worker['tests_run']!=EXPECTED_TEST_COUNT or type(worker['test_ids']) is not list
    ...
    for key in ('failures','errors','skipped','expected_failures','unexpected_successes'):
        if worker[key]!=0:raise ValueError('test worker '+key+' is nonzero')
    if worker['loader_errors']!=[] or worker['return_code']!=0:
...
def _run_test_suite():
    ...
    passed=(tested.testsRun==EXPECTED_TEST_COUNT==len(ids)>0 and len(set(ids))==len(ids)
            ...
            and files==list(EXPECTED_TEST_FILES) and tested.wasSuccessful()
...
def _test_worker(identity):
    ...
    completed=subprocess.run(command,cwd=str(HERE.parents[1]),capture_output=True,timeout=180)
