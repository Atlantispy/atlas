...
SOURCE_PINS = {
    'continuous_pool.py':'7064eebff7e051a8fa58f48ed56d46f90f6241445a1b0ff6c80e72a2d5c9d6d8',
    'FORCED_DRYING_DESIGN.md':'29700658dd0b1576a07e6f57ab88168e2d6e7faba1884431f3975ab2fae14181',
}
...
def _source_snapshot(implementation_bytes=None):
    bound_module=(_BASE/'continuous_pool.py').resolve()
    loaded_path=getattr(continuous_pool,'__file__',None)
    if loaded_path is None or Path(loaded_path).resolve() != bound_module:
        raise ForcedDryingError('loaded continuous-pool module is not the bound source path')
    records=[]
    for name,expected in SOURCE_PINS.items():
        path=_BASE/name;data=path.read_bytes();digest=hashlib.sha256(data).hexdigest()
        if digest != expected:
            raise ForcedDryingError('bound forced-drying source identity mismatch: '+name)
...
def first_event(*, liquid_m3, suspended_solid_m3, total_wet_area_m2,
    ...
    def check_resources():
        if time.monotonic() > deadline:
            raise ForcedDryingError('forced-drying cooperative wall-time envelope exceeded')

    def probe(t):
        nonlocal scalar_calls, scalar_evaluations, source_rechecks
        check_resources()
        if scalar_calls >= max_scalar_calls:
            raise ForcedDryingError('forced-drying scalar-call envelope exhausted')
        scalar_calls += 1
        _source_snapshot(implementation_bytes);source_rechecks+=1
        try:
            result = continuous_pool.advance_pool(w0, s0, area, qw, qs, qo, velocity, t)
            ...
            if scalar_evaluations > max_scalar_evaluations:
                raise ForcedDryingError('forced-drying scalar-evaluation envelope exhausted') from exc
        ...
        if scalar_evaluations > max_scalar_evaluations:
            raise ForcedDryingError('forced-drying scalar-evaluation envelope exhausted')
        ...
        row = {'time_years':t, 'status':'PASS', 'height_m':height,
               ...
               'height_error_bound_m':height_error, 'concentration':concentration,
               'concentration_error_bound':concentration_error,
               'depth_rate_m_year':slope, 'depth_rate_error_bound_m_year':slope_error,
    ...
    def no_event(reason, minimum):
        return finish({**base, 'status':'NO_DRYING_CERTIFIED', 'reason':reason,
                       'minimum_certificate':minimum, 'remaining_years':duration,
                       'physical_acceptance':False, 'production_authorised':False,
                       'daughter_geometry_solved':False, 'remaining_interval_completed':True})
    ...
    def crossed(low, high):
        ...
        return finish({**base,'status':'FIRST_DRYING_EVENT','reason':'first descending root bracketed',
            ...
            'physical_acceptance':False,'production_authorised':False,
            'daughter_geometry_solved':False,'remaining_interval_completed':remaining == 0})
    ...
    for _ in range(MAX_BISECTIONS):
        ...
        if hi < 0:low=mid
            ...
            if (mid['height_m'] == 0 and mid['height_error_bound_m'] == 0
    ...
    if minimum_lower > 0:
        return no_event('convex interior minimum has a positive conservative lower bound',minimum)
    if (minimum_probe['height_m'] == 0 and minimum_probe['height_error_bound_m'] == 0
