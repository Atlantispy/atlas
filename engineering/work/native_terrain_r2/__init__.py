"""Versioned bounded numerical successor; predecessor sources stay untouched."""
import hashlib
...
if sys._getframe().f_code != compile(_SOURCE, __file__, 'exec', dont_inherit=True):
    raise ValueError('executed native numerical bootstrap differs from source')
...
class _Finder(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        ...
        if not name.isidentifier():
            raise ImportError('flat native numerical module identity required')
        source = HERE / (name + '.py')
        if not source.is_file():
            raise ImportError('undeclared native numerical module: ' + fullname)
