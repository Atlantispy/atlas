"""Quality-preserving execution of an unchanged R11 snapshot graph.
...
execute(jobs) returns {stage_id: {product, artifacts, diagnostics}}. Both extras
are mappings; on_restore installs/validates them before downstream execution.
...
"""
    ...
    for ready in _frontiers(nodes, order[prefix_count:until], rows):
