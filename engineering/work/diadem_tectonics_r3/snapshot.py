...
R3 decodes each consumed package file once per invocation. Optional external pins
bind those same initial bytes; completion rechecks raw bytes, size and plain paths.
No parsed package state is supplied by callers or retained between invocations.
"""
...
def _raw(path: Path) -> tuple[bytes, dict]:
    """Read bounded bytes from a plain file without decoding JSON."""
    _plain(path)
    ...
    return raw, {"file": path.name, "bytes": len(raw),
                 "sha256": hashlib.sha256(raw).hexdigest()}


def _read(path: Path, *, expected_sha256: str | None = None) -> tuple[dict, dict]:
    raw, pin = _raw(path)
    if expected_sha256 is not None and pin["sha256"] != expected_sha256:
        raise SnapshotError(f"tectonic input changed; no repin: {path.name}")
    try:
...
def _package_pins(package_pins: Any) -> dict[str, str]:
    """Validate and detach the complete external filename/hash inventory."""
    names = {MANIFEST, *FILES.values()}
    if type(package_pins) is not dict or set(package_pins) != names:
        raise SnapshotError("all consumed tectonic package file pins required")
    if any(type(value) is not str or not re.fullmatch(r"[0-9a-f]{64}", value)
           for value in package_pins.values()):
        raise SnapshotError("package SHA-256 must be lowercase hexadecimal")
    return dict(package_pins)


def verify_package(package_dir: Path, package_pins: dict[str, str]) -> None:
    """Authenticate current package bytes against complete external pins, raw-only.

    This checks identity, file bounds and plain paths. It does not establish JSON
    validity or manifest semantics; callers restoring results must separately
    authenticate the previously validated result and its source identity.
    """
    expected = _package_pins(package_pins)
    package = Path(package_dir).absolute()
    _plain(package)
    for name in (MANIFEST, *FILES.values()):
        if _raw(package / name)[1]["sha256"] != expected[name]:
            raise SnapshotError(f"tectonic input changed; no repin: {name}")
...
def build_snapshot(package_dir: Path, domain_decisions: dict | None = None,
                   *, package_pins: dict[str, str] | None = None) -> dict:
    ...
    Optional package_pins are exact filename/hash bindings, not decoded inputs.
    """
    expected_pins = {} if package_pins is None else _package_pins(package_pins)
    package = Path(package_dir).absolute()
    _plain(package)
    manifest, manifest_pin = _read(package / MANIFEST, expected_sha256=expected_pins.get(MANIFEST))
    try:
    ...
    for role, name in FILES.items():
        payload, pin = _read(package / name, expected_sha256=expected_pins.get(name))
        try:
    ...
    for pin in pins:
        if _raw(package / pin["file"])[1] != pin:
