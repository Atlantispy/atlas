"""Source-bound R10 entry point; sealed biome, soil and physical parents are immutable.
...
RECIPE_SCHEMA = 'diadem.seasonal-world-recipe.r10'
RESULT_SCHEMA = 'diadem.seasonal-world-result.r10'
CHECKPOINT_SCHEMA = 'diadem.seasonal-world-checkpoint.r10'
...
def module_nodes(sources):
    ...
    for path,digest in sources.items():
        ...
        if source.parent!=provenance.HERE:
            raise ValueError('bounded R10 Python module layout must be flat')
        logical='work.generator_upgrade_r10.'+source.stem
        if logical in nodes: raise ValueError('duplicate R10 private module identity')
...
class Bundle:
    def __init__(self):
        ...
        self.graph=self.parent.graph.__class__(module_nodes(self.identity['r10_sources']))

    def verify(self):
        self.graph.verify()
        current,digest=provenance.source_identity(self.parent)
        if current!=self.identity or digest!=self.source_sha256:
            raise ValueError('R10 source inventory changed after binding')
        return digest

    @property
    def pipeline(self):
        return self.graph.load('work.generator_upgrade_r10.pipeline')

    @property
    def reference(self):
        return self.graph.load('work.generator_upgrade_r10.reference')
    ...
    def checkpoint(self, result):
        ...
        if (type(result) is not dict or result.get('schema')!=RESULT_SCHEMA
            ...
            raise ValueError('this exact source-bound R10 result required')
