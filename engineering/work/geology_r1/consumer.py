...
from work.generator_upgrade_r18 import provenance as retained_provenance
from .native import regional, erosion, terrain, guarded
...
def verify(snapshot):
    ...
    if binding.get('schema') == 'diadem.geological-coverage-execution-binding.r18':
        guarded(retained_provenance, 'verify', binding)
    elif binding.get('schema') == 'diadem.geology-optimisation-execution.r1':
        p.verify(binding)
    else:
        raise ValueError('known retained or G1 geological source binding required')
