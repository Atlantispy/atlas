...
from work.generator_upgrade_r16 import columns as quantities
from .native import regional
...
"""
    ...
    interpretations = {}
    used = set()
    prepared_palette = composite._prepare_palette(palette)
    for key, point in supports.items():
        ...
        for index, layer in enumerate(row['layers']):
            ...
            material = composite._validated(mid, prepared_palette)
    ...
    science['regional_input'] = {'schema': 'diadem.geological-coverage-input.r18',
        ...
        'constituent_balances': accounts.project_balances(science['construction']['material_accounts'], prepared_palette, accounts.CONSTRUCTION_STAGES),
    ...
    composite._check_palette(palette, prepared_palette)
    p.verify(binding)
    return result
