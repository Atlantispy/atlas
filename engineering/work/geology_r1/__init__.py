...
if sys._getframe().f_code != compile(_SOURCE, __file__, 'exec', dont_inherit=True):
    raise ValueError('executed Geology R1 bootstrap differs from current source')
...
class _Finder(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        ...
        if not name.isidentifier():
            raise ImportError('flat Geology R1 module identity required')
        source = HERE/(name+'.py')
        if not source.is_file():
            raise ImportError('undeclared Geology R1 module: '+fullname)
