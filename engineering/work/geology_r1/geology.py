...
def prepare(programme):
    """Detach owner instructions once; programme.build remains plain JSON.

This object is scoped to a caller's source-bound operation, never persisted.
Raw sample ranges and every runtime arithmetic intermediate remain enforced.
"""
    if type(programme) is _PreparedProgramme:
        return programme
    plan = deepcopy(programme)
    for name, bounds in plan['field_ranges'].items():
        lower, upper = bounds
        plan['field_ranges'][name] = [columns._q(lower, 'source lower bound'),
            None if upper is None else columns._q(upper, 'source upper bound')]
    for row in plan['derived_fields']:
        row['expression'] = rules.prepare(row['expression'])
    for stage in plan['stages']:
        if stage['kind'] == 'emplace':
            stage['thickness_m'] = rules.prepare(stage['thickness_m'])
            stage['weights'] = {unit: rules.prepare(expr) for unit, expr in stage['weights'].items()}
        elif stage['kind'] == 'replace':
            stage['depth_m'] = rules.prepare(stage['depth_m'])
            stage['supports'] = {unit: rules.prepare(expr) for unit, expr in stage['supports'].items()}
            if 'coverage' in stage:
                stage['coverage'] = rules.prepare(stage['coverage'])
        else:
            raise ValueError('unsupported owner construction stage')
    for name in ('base_m', 'translation_m'):
        plan[name] = rules.prepare(plan[name])
    return _PreparedProgramme(_freeze(plan))
...
"""
    ...
    for key, raw in samples.items():
        ...
        for name, value in raw.items():
            ...
            if q < lower or (upper is not None and q > upper):
        ...
        for stage in programme['stages']:
            ...
            if stage['kind'] == 'emplace':
                ...
                if height:
                    ...
                    layer = {'compartment_id': identity, 'thickness_m': height, 'weights': weights}
                    layers.append(layer)
                ...
                layers, transfers = replacement._replace(layers, depth, replacements,
                ...
                history.append({'stage_id': identity, 'kind': 'replace', 'depth_m': str(depth),
                                'transfers': replacement._plain_transfers(transfers)})
            else:
                raise ValueError('unsupported owner construction stage')
        ...
        for layer in layers:
            ...
            native_layers.append({'compartment_id': layer['compartment_id'], 'material_id': mid,
                                  'thickness_m': str(layer['thickness_m'])})
