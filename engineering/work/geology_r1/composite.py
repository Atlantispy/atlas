...
def _prepare_palette(palette, material_ids=None):
    """Fully validate and detach each used descriptor once per operation.

Evidence and omitted-zero metadata participate in full validation even though
the material hash deliberately excludes them. No cross-call ID-only cache.
"""
    if type(palette) is _PreparedPalette:
        return palette
    if type(palette) is not dict:
        raise ValueError('bound composite palette entry required')
    descriptors, projections = {}, {}
    for mid in (palette if material_ids is None else dict.fromkeys(material_ids)):
        descriptor = _validated(mid, palette)
        descriptors[mid] = _freeze(descriptor)
        projections[mid] = (
            columns._q(descriptor['grain_density_kg_m3'], 'effective grain density', positive=True),
            tuple((row['unit_id'], columns._q(row['mass_fraction'], 'constituent mass fraction'),
                   columns._q(row['grain_density_kg_m3'], 'constituent grain density', positive=True))
                  for row in descriptor['constituents']))
    return _PreparedPalette(MappingProxyType(descriptors), MappingProxyType(projections))


def _check_palette(palette, prepared):
    """A mutable public palette must still match the complete operation copy."""
    if type(palette) is _PreparedPalette:
        return
    if type(palette) is not dict or any(
            mid not in palette or _freeze(palette[mid]) != descriptor
            for mid, descriptor in prepared.descriptors.items()):
        raise ValueError('composite descriptor changed during operation')
...
def _validated(material_id, palette):
    ...
    if type(palette) is _PreparedPalette:
        if material_id not in palette.descriptors:
            raise ValueError('bound composite palette entry required')
        return palette.descriptors[material_id]
...
"""
    prepared = _prepare_palette(palette, (material_id,))
    result = _project_mass(material_id, mass_kg, prepared)
    _check_palette(palette, prepared)
    return {unit: {key: str(value) for key, value in row.items()} for unit, row in result.items()}


def _project_mass(material_id, mass_kg, palette):
    """Exact internal projection; callers own one validated immutable palette."""
    _validated(material_id, palette)
    mass = columns._q(mass_kg, 'composite stock mass', nonnegative=True)
    density, constituents = palette.projections[material_id]
    result = {}
    for unit, fraction, unit_density in constituents:
        unit_mass = columns._q(mass*fraction, 'projected constituent mass')
        result[unit] = {'mass_kg': unit_mass,
            'solid_volume_m3': columns._q(unit_mass/unit_density, 'projected constituent solid volume')}
    mass_sum = columns._sum((row['mass_kg'] for row in result.values()), 'projected mass sum')
    solid_sum = columns._sum((row['solid_volume_m3'] for row in result.values()), 'projected solid sum')
