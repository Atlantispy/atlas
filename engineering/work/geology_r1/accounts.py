...
"""
    rows = list(rows)
    prepared = composite._prepare_palette(palette, (row['material_id'] for row in rows))
    totals = {}
    for row in rows:
        mid = row['material_id']
        for label, mass_key, solid_key, sign in stages:
            mass, solid = quantity(row[mass_key]), quantity(row[solid_key])
            projected = composite._project_mass(mid, mass, prepared)
            if sum((v['mass_kg'] for v in projected.values()), F()) != mass:
                raise ArithmeticError('projected constituent mass differs from native account')
            if sum((v['solid_volume_m3'] for v in projected.values()), F()) != solid:
            ...
            for unit, values in projected.items():
                ...
                for suffix, source in (('mass_kg', 'mass_kg'), ('solid_volume_m3', 'solid_volume_m3')):
                    key = label+'_'+suffix
                    target[key] = columns._q(target.get(key, F())+values[source], 'summed constituent stock')
    answer = []
