...
"""
    ...
    for row in layers:
        ...
        original.append({'compartment_id': identity, 'thickness_m': str(thickness),
                         'weights': {unit: str(weight) for unit, weight in weights.items()}})
    total = columns._sum((columns._q(row['thickness_m'], 'compartment thickness') for row in original), 'finite column height')
    boundary = columns._q(total-min(depth, total), 'replacement boundary above base')
    answer, transfers = [], []
    bottom = columns._q(0, 'interval bottom')
    for row in original:
        thickness = columns._q(row['thickness_m'], 'compartment thickness')
        ...
        weights = {unit: columns._q(value, 'resident bulk weight') for unit, value in row['weights'].items()}
        ...
        if retained_height:
            ...
            answer.append({'compartment_id': lower_id, 'thickness_m': str(retained_height),
        ...
        answer.append({'compartment_id': new_id, 'thickness_m': str(affected),
                       'weights': {unit: str(value) for unit, value in modified.items()}})
        transfers.append({'source_compartment_id': row['compartment_id'], 'compartment_id': new_id,
            'thickness_m': str(affected), 'depth_start_m': str(columns._q(total-top, 'segment start depth')),
            'depth_stop_m': str(columns._q(total-active_bottom, 'segment stop depth')),
            'coverage': str(cover), 'eligible_bulk_fraction': str(host_sum),
            'removed_bulk_thickness_m': {unit: str(value) for unit, value in removed.items()},
            'introduced_bulk_thickness_m': {unit: str(value) for unit, value in introduced.items()}})
        bottom = top
    ...
    if columns._sum((columns._q(row['thickness_m'], 'final compartment thickness') for row in answer), 'final column height') != total:
        raise ArithmeticError('replacement changed finite bulk height')
    before, after = _inventory(original), _inventory(answer)
    ...
    for transfer in transfers:
        for values, target in ((transfer['removed_bulk_thickness_m'], removed_totals),
            ...
            for unit, value in values.items():
                target[unit] = columns._q(target.get(unit, 0)+columns._q(value, 'bulk transfer'), 'total bulk transfer')
