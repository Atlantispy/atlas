"""Bounded 2-D material-conservative terrain reference, not a canon authority.
...
def gradients(grid, heights):
    ...
    for i in range(grid.size):
        ...
        gx.append((z[right]-z[left])/((right-left)*grid.dx_m))
    ...
    return {"dz_dx":gx,"dz_dy":gy,"grade_m_per_m":norm,
            ...
            "grade_percent":[100*s for s in norm],"edge_method":"one-sided exterior; centred interior"}
...
def hillslope_step(state, diffusivity_m2_year, dt_years, critical_gradient=None, *, periodic_x=False):
    ...
    for i in range(n):
        ...
        for j,distance,width,tangent in ((right,g.dx_m,g.dy_m,"dz_dy"),(i+g.cols,g.dy_m,g.dx_m,"dz_dx")):
            ...
            amount=coefficient*abs(normal)/(1-ratio)*width*dt*(1-state.porosity[source])
